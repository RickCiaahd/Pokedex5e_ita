...
void main() {
  ...
  test('imports every section with a new ID and deletes it completely', () async {
    ...
    await pokedexRepository.saveEntry(
      profileId: source.id,
      entry: PokedexEntry.empty(25).setFormState(
        formName: null,
        seen: true,
        caught: true,
      ),
    ...
    expect((await pcRepository.getPokemon(imported.id)).single.nickname, 'Bulby');
    ...
    expect(
      (await teamRepository.getTeam(imported.id)).every(
        (slot) => slot.pokemonId == null,
      ),
      isTrue,
    );
